...
function headOptions() {
	...
	// NY-9_may_Victory_Day
    //document.head.classList.add('oldvk-9_may_Victory_Day-' + getRandomInt(1, 6).toString());
	// NY-martha
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addTopMenu() {
        // 🔴 БЫЛО: getElementById('top_nav')
        var top_nav = document.querySelector('.TopNavigationWrapper__inner--8vD9H');

        if (top_nav && !document.getElementById('oldvk_top_menu')) {
            var top_menu = `
                <div class="head_nav_item fl_r">
                    <a id="oldvk_top_exit" class="top_nav_link" href="">
                        <div class="top_profile_name"></div>
                    </a>
                </div>

                <div class="head_nav_item fl_r">
                    <a id="oldvk_top_help" class="top_nav_link" href="/support?act=home">
                        <div class="top_profile_name"></div>
                    </a>
                </div>

                <div class="head_nav_item fl_r">
                    <a id="oldvk_top_music" class="top_nav_link" href="">
                        <div class="top_profile_name">${i18n.music[lang]}</div>
                        <div id="oldvk_top_play" class="oldvk-hide"></div>
                    </a>
                </div>

                <div class="head_nav_item fl_r">
                    <a id="oldvk_top_apps" class="top_nav_link" href="/apps">
                        <div class="top_profile_name">${i18n.games[lang]}</div>
                    </a>
                </div>

                <div class="head_nav_item fl_r">
                    <a id="oldvk_top_communities" class="top_nav_link" href="/search?c[section]=communities">
                        <div class="top_profile_name">${i18n.communities[lang]}</div>
                    </a>
                </div>

                <div class="head_nav_item fl_r">
                    <a id="oldvk_top_peoples" class="top_nav_link" href="/search?c[section]=people">
                        <div class="top_profile_name">${i18n.people[lang]}</div>
                    </a>
                </div>
            `;
            ...
            if (oldvk_top_exit && top_logout_link) {
                oldvk_top_exit.querySelector('.top_profile_name').textContent =
                    top_logout_link.textContent.toLowerCase();
            ...
            if (oldvk_top_help && top_support_link) {
                oldvk_top_help.querySelector('.top_profile_name').textContent =
                    top_support_link.textContent.toLowerCase();
            }
    ...
    function waitForElementsAndAddMenu() {
        const logout = document.getElementById('top_logout_link');
        const support = document.getElementById('top_support_link');
        const nav = document.querySelector('.TopNavigationWrapper__inner--8vD9H');

        if (logout && support && nav) {
            addTopMenu();
        } else {
            setTimeout(waitForElementsAndAddMenu, 200);
        }
    }

    waitForElementsAndAddMenu();
});
...
KPP.add('#react_rootprofile', function () {
    ...
    function translateTimeUnit(unit, number){

        if(!timeTranslations[unit]) return unit;

        return pluralRu(number, timeTranslations[unit].ru);
    ...
    function getProfileName(){
        ...
        if(!ownerPageName) return "";

        return ownerPageName.textContent
            .replace(/\s+/g," ")
            .replace(/онлайн.*/i,"")
            .replace(/заходил.*/i,"")
            .trim();
    ...
    function getGender(){
        ...
        var ownerPageName = document.getElementById('owner_page_name');
        if(!ownerPageName) return "unknown";

        var text = ownerPageName.textContent;

        if(text.includes("заходила")) return "female";
        if(text.includes("заходил")) return "male";

        return "unknown";

    }

    function updateLastOnlineTime(element){
        ...
        var pronoun = "заходил(а)";
        if(gender === "female") pronoun = "заходила";
        if(gender === "male") pronoun = "заходил";
        ...
        if(lastSeenElement){
            ...
            .replace(/(\d+)\s*д/g,(m,p)=>p+" "+translateTimeUnit("д",p))
            .replace(/(\d+)\s*ч/g,(m,p)=>p+" "+translateTimeUnit("ч",p))
            .replace(/(\d+)\s*мин/g,(m,p)=>p+" "+translateTimeUnit("мин",p))
            .replace(/(\d+)\s*с/g,(m,p)=>p+" "+translateTimeUnit("с",p));

            element.textContent = pronoun + " " + detailedTime + " назад";

            lastOnlineTime[profileName] = Date.now();

        }
        else if(lastOnlineTime[profileName]){

            var date = new Date(lastOnlineTime[profileName]);

            element.textContent = pronoun + " " + date.toLocaleString(getLanguage());

        }
        else{

            element.textContent = "был давно";
    ...
    function insertProfileTitle(){
        ...
        if(document.getElementById('oldvk_profile_title')) return;

        var ownerPageName = document.getElementById('owner_page_name');
        if(!ownerPageName) return;

        var cleanText = getProfileName();
        ...
        var wrapElement = document.getElementById('profile_redesigned');
        ...
        updateLastOnlineTime(lastOnlineDiv);
            ...
            mutations.forEach(function(mutation){
                ...
                mutation.addedNodes.forEach(function(node){
                    ...
                    if(node.nodeType === 1 && (node.matches('.ProfileWrapper') || node.querySelector('.ProfileWrapper'))){
                        ...
                        setTimeout(insertProfileTitle,500);
