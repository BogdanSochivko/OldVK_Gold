...
function addTopMenu() {
    ...
    const top_nav = document.querySelector('[class*="TopNavigationWrapper"]');
    if (!top_nav) return;

    if (document.getElementById("oldvk_top_menu")) return;

    const menu = document.createElement("div");
    menu.id = "oldvk_top_menu";

    menu.innerHTML = `
    <div class="head_nav_item fl_r">
        <a id="oldvk_top_exit" class="top_nav_link">
            <div class="top_profile_name">выход</div>
        </a>
    </div>

    <div class="head_nav_item fl_r">
        <a id="oldvk_top_help" class="top_nav_link" href="/support?act=home">
            <div class="top_profile_name">помощь</div>
        </a>
    </div>

    <div class="head_nav_item fl_r">
        <a id="oldvk_top_music" class="top_nav_link" href="/audio">
            <div class="top_profile_name">музыка</div>
        </a>
    </div>

    <div class="head_nav_item fl_r">
        <a id="oldvk_top_apps" class="top_nav_link" href="/apps">
            <div class="top_profile_name">игры</div>
        </a>
    </div>

    <div class="head_nav_item fl_r">
        <a id="oldvk_top_communities" class="top_nav_link" href="/groups">
            <div class="top_profile_name">сообщества</div>
        </a>
    </div>

    <div class="head_nav_item fl_r">
        <a id="oldvk_top_peoples" class="top_nav_link" href="/search?c[section]=people">
            <div class="top_profile_name">люди</div>
        </a>
    </div>
    `;

    top_nav.appendChild(menu);

    // logout
    const logout = document.getElementById("top_logout_link");
    const exit = document.getElementById("oldvk_top_exit");

    if (logout && exit) {
        exit.href = logout.href;
        exit.querySelector(".top_profile_name").textContent =
            logout.textContent.trim().toLowerCase();
    }

    // help
    const support = document.getElementById("top_support_link");
    const help = document.getElementById("oldvk_top_help");

    if (support && help) {
        help.querySelector(".top_profile_name").textContent =
            support.textContent.trim().toLowerCase();
    }

    // music placement
    const music = document.getElementById("oldvk_top_music");
    const audioPlace = document.getElementById("top_audio_layer_place");

    if (music && audioPlace) {
        insertAfter(music, audioPlace);
    }

    console.log("OldVK menu added");
}
...
KPP.add('#react_rootprofile', function (element) {
